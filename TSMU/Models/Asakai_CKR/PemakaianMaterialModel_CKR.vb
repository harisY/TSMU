...
Public Class PemakaianMaterialModel_CKR
    ...
    Public Property ObjDetailsMaterial() As New Collection(Of PemakaianMaterialDetail_CKR)
    Public Property ObjDetailsKomponen() As New Collection(Of PemakaianKomponenlDetail_CKR)
            ...
            Else
                ...
                Else
                    ...
                    If Len(IDTrans) = 1 Then
                        IDTrans = "M" & Tahun & Bulan & "000" & IDTrans & ""
                    ElseIf Len(IDTrans) = 2 Then
                        IDTrans = "M" & Tahun & Bulan & "00" & IDTrans & ""
                    ElseIf Len(IDTrans) = 3 Then
                        IDTrans = "M" & Tahun & "0" & IDTrans & ""
                    Else
                        IDTrans = "M" & Tahun & Bulan & IDTrans & ""
                    End If
    ...
    Public Function GetDataDetailMaterial(ID As String) As DataTable
        Try
            Dim query As String = "SELECT 
                                    ...
                                    FROM AsakaiPemakaianMaterialDetail Left join New_BOM.dbo.Inventory on AsakaiPemakaianMaterialDetail.IDMaterial =Inventory.InvtID where AsakaiPemakaianMaterialDetail.IDTrans  = '" & ID & "'"
            Dim dtTable As New DataTable
            'dtTable = MainModul.GetDataTableByCommand(query)
            dtTable = GetDataTableByParam(query, CommandType.Text, Nothing, GetConnStringDbCKR)
    ...
    Public Function GetDataDetailKomponen(ID As String) As DataTable
        Try
            Dim query As String = "SELECT 
                                    ...
                                    FROM AsakaiPemakaianKomponenDetail Left join New_BOM.dbo.Inventory on AsakaiPemakaianKomponenDetail.IDMaterial =Inventory.InvtID where AsakaiPemakaianKomponenDetail.IDTrans  = '" & ID & "'"
            Dim dtTable As New DataTable
            'dtTable = MainModul.GetDataTableByCommand(query)
            dtTable = GetDataTableByParam(query, CommandType.Text, Nothing, GetConnStringDbCKR)
...
#Region "CRUD"
    ...
    Public Sub DeleteHeader(ByVal ID As String)
        Try
            ...
            ExecQueryWithValue(ls_PMK, CommandType.Text, Nothing, GetConnStringDbCKR)
    ...
    Public Sub DeleteDetail(ByVal ID As String)
        Try
            ...
            'ExecQuery(ls_SPD)
            ExecQueryWithValue(ls_PM, CommandType.Text, Nothing, GetConnStringDbCKR)

            Dim ls_PMD As String = "DELETE FROM AsakaiPemakaianMaterialDetail WHERE rtrim(IDTrans)=" & QVal(ID) & ""
            'MainModul.ExecQuery(ls_SP)
            ExecQueryWithValue(ls_PMD, CommandType.Text, Nothing, GetConnStringDbCKR)

            Dim ls_PK As String = "DELETE FROM AsakaiPemakaianKomponen WHERE rtrim(IDTrans)=" & QVal(ID) & ""
            'ExecQuery(ls_SPD)
            ExecQueryWithValue(ls_PK, CommandType.Text, Nothing, GetConnStringDbCKR)

            Dim ls_PKD As String = "DELETE FROM AsakaiPemakaianKomponenDetail WHERE rtrim(IDTrans)=" & QVal(ID) & ""
            'ExecQuery(ls_SPD)
            ExecQueryWithValue(ls_PKD, CommandType.Text, Nothing, GetConnStringDbCKR)
...
Public Class PemakaianMaterialDetail_CKR
    Dim _Query As String
...
Public Class PemakaianKomponenlDetail_CKR
    Dim _Query As String
