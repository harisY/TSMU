...
Public Class TTI_Header_Models
  ...
  FROM [bankreceipt] WHERE Proses=0 AND CustID!=''"
    ...
    Public Function GetDataGrid3() As DataTable
      ...
      ,[CustomerName]
                ...
                ,[cmdm_manual]
                ,[cmdm_manual_ket]
            FROM [TTI_Header] where [Total_DPP_PPN]-[PPh]-[Biaya_Transfer]-[CM_DM]!=0 Order By id desc"
            Dim dt As New DataTable
            dt = MainModul.GetDataTable_Solomon(sql)
            Return dt
        Catch ex As Exception
    ...
    Public Sub GetPaymentByVoucherNo()
        Try
            Dim sql As String = "SELECT [id]
                    ...
                    ,[cmdm_manual_ket]
                    ,[TotReceive]
                    FROM [TTI_Header] where id=" & QVal(id) & ""
            Dim dt As New DataTable
            dt = MainModul.GetDataTable_Solomon(sql)
  ...
  FROM [bankreceipt] where NoBukti=" & QVal(NoBukti) & ""
    ...
    Public Sub InsertHeaderDir()
        Try
            Dim ls_SP As String = " " & vbCrLf &
                                    "INSERT INTO TTI_Header (cek4) " & vbCrLf &
                                    "Values(" & QVal(Me.cek4) & ")"
            MainModul.ExecQuery_Solomon(ls_SP)
        Catch ex As Exception
            Throw ex
    ...
    Public Function GetDataGridApprove(ByVal Level As Integer) As DataTable
        Dim sql As String = "SELECT 
                            ...
                            FROM TTI_Header "
        Try
            '' query = "Select vrno as No_Voucher,CustomerName as Supplier,Total_DPP_PPN+PPh+Biaya_Transfer as Amount, cek3 as Check1, cek4 as Check2 from TTI_Header"
            If Level = 1 Then
	...
	SELECT 
                                TTI_Header.id
                                ,TTI_Header.vrno as VoucherNo
                                ,TTI_Header.tgl Tanggal
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
                                ,TTI_Header.CustomerName
                                ,(sum(payment_detail1.dpp)+sum(payment_detail1.Ppn)-sum(payment_detail1.pph))-TTI_Header.cm_dm-TTI_Header.Biaya_Transfer as PaidAmount
                                , TTI_Header.cek1 as Level1, TTI_Header.cek2 as Level2, TTI_Header.cek3 as Level3, TTI_Header.cek4 as Direktur 
                            FROM payment_detail1  inner join TTI_Header on payment_detail1.vrno=TTI_Header.vrno 
...
"
        Try
            '' query = "Select vrno as No_Voucher,CustomerName as Supplier,Total_DPP_PPN+PPh+Biaya_Transfer as Amount, cek3 as Check1, cek4 as Check2 from TTI_Header"
            If Level = 1 Then
                sql = sql & " WHERE TTI_Header.BankID = COALESCE(NULLIF('" & BankID & "',''),BankID) group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
								...
								,TTI_Header.cek1, 
								TTI_Header.cek2, 
								TTI_Header.cek3, TTI_Header.cek4 
								,TTI_Header.cm_dm
								,TTI_Header.Biaya_Transfer
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno"
            ElseIf Level = 2 Then
                sql = sql & " WHERE TTI_Header.BankID = COALESCE(NULLIF('" & BankID & "',''),BankID) AND TTI_Header.cek1 ='1' AND TTI_Header.cek2='0' AND TTI_Header.cek3='0' AND TTI_Header.cek4='0' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
								...
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno"
            ElseIf Level = 3 Then
                sql = sql & " WHERE TTI_Header.BankID = COALESCE(NULLIF('" & BankID & "',''),BankID) AND TTI_Header.cek1 ='1' AND TTI_Header.cek2='1' AND TTI_Header.cek3='0' AND TTI_Header.cek4='0' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
								...
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno"
            Else
                sql = sql & " WHERE TTI_Header.BankID = COALESCE(NULLIF('" & BankID & "',''),BankID) AND TTI_Header.cek1 ='1' AND TTI_Header.cek2='1' AND TTI_Header.cek4='0' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
    ...
    Public Function GetDataGridReject1(ByVal Level As Integer) As DataTable
        Dim sql As String = "select TTI_Header.id
                                ,TTI_Header.vrno as VoucherNo
                                ,TTI_Header.tgl Tanggal
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
            ...
            If Level = 1 Then
                sql = sql & " where payment_detail1.cek1='0'
							 group by TTI_Header.id
								...
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno "
            ElseIf Level = 2 Then
                sql = sql & " where payment_detail1.cek1='0' and  payment_detail1.cek2='0' and TTI_Header.cek4=0
							 group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
								...
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno "
            ElseIf Level = 3 Then
                sql = sql & " where payment_detail1.cek1='0' and  payment_detail1.cek3='0' and TTI_Header.cek4=0
							 group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
								...
								ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno "
            ElseIf Level = 4 Then
                sql = sql & " where payment_detail1.cek1='0' and  payment_detail1.cek4='0' and TTI_Header.cek4=0
							 group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
    ...
    Public Function GetDataGridApproveDone(ByVal Level As Integer) As DataTable
        ...
        Try
            '' query = "Select vrno as No_Voucher,CustomerName as Supplier,Total_DPP_PPN+PPh+Biaya_Transfer as Amount, cek3 as Check1, cek4 as Check2 from TTI_Header"
            If Level = 2 Then
                sql = sql & " WHERE TTI_Header.cek2='1' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
							...
							ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno"
            ElseIf Level = 3 Then
                sql = sql & " WHERE TTI_Header.cek3='1' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
							...
							ORDER BY TTI_Header.tgl, TTI_Header.Customername, TTI_Header.vrno"
            ElseIf Level = 4 Then
                sql = sql & " WHERE TTI_Header.cek4='1' group by TTI_Header.id
                                ,TTI_Header.vrno 
                                ,TTI_Header.tgl
                                ,TTI_Header.BankName
                                ,TTI_Header.CuryID
    ...
    Public Sub UpdateCek(ByVal Level As Integer)
        Try
            ...
            If Level = 2 Then
                ls_SP = "UPDATE TTI_Header SET cek2= " & QVal(True) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            ElseIf Level = 3 Then
                ls_SP = "UPDATE TTI_Header SET cek3= " & QVal(True) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            ElseIf Level = 4 Then
                ls_SP = "UPDATE TTI_Header SET cek4= " & QVal(True) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            End If
            MainModul.ExecQuery_Solomon(ls_SP)
        Catch ex As Exception
            Throw
    ...
    Public Sub UpdateUnCek(ByVal Level As Integer)
        Try
            Dim ls_SP As String = String.Empty
            If Level = 1 Then
                ls_SP = "UPDATE TTI_Header SET cek1= " & QVal(False) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            ElseIf Level = 2 Then
                ls_SP = "UPDATE TTI_Header SET cek2= " & QVal(False) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            ElseIf Level = 3 Then
                ls_SP = "UPDATE TTI_Header SET cek3= " & QVal(False) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            ElseIf Level = 4 Then
                ls_SP = "UPDATE TTI_Header SET cek4= " & QVal(False) & " WHERE vrno=" & QVal(vrno.TrimEnd) & ""
            End If
            MainModul.ExecQuery_Solomon(ls_SP)
        Catch ex As Exception
            Throw
