Public Class frmBoM_detail
    ...
    Private Sub GridDetail_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridDetail.CellContentDoubleClick
    ...
    Sub populateRoot()
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                Dim str As String = "level 0 - " & dt.Rows(0)("FinishedGood").ToString
                node = TreeLevel.Nodes.Add(str)
                Dim tag As String = dt.Rows(0)("FinishedGood").ToString()
                node.Tag = tag
                'node.ImageIndex = ds.Tables(0).Rows(i)("imageindex")
                'node.SelectedImageIndex = ds.Tables(0).Rows(i)("imageindex")
                node.Name = dt.Rows(0)("FinishedGood")
                node.Text = str
            End If
    ...
    Sub PopulateLvltwo()
        ...
        Try
            ...
            TreeLevel.BeginUpdate()
            'For Each node As TreeNode In TreeHak.Nodes
                ...
                For i As Integer = 0 To TreeLevel.Nodes(idx).Nodes.Count - 1
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        For rw = 0 To dt.Rows.Count - 1
                            ...
                            'If dt.Rows(rw)("invtId").ToString = str Then
                            'Else
                            '    Dim node As New TreeNode
                            '    str = dt.Rows(rw)("invtId").ToString
                            '    node = TreeLevel.Nodes(idx).Nodes(i).Nodes.Add(str)
                            '    node.Tag = dt.Rows(rw)("invtId").ToString()
                            'End If
                        Next rw
                ...
                For i As Integer = 0 To TreeLevel.Nodes(0).Nodes(idx).Nodes.Count - 1
                    If dtTable.Rows.Count > 0 Then
                        For rw = 0 To dtTable.Rows.Count - 1
                            ...
                            End If
                            'If dtTable.Rows(rw)("invtId").ToString = str Then
                            'Else
                            '    Dim node As New TreeNode
                            '    str = dtTable.Rows(rw)("invtId").ToString
                            '    node = TreeLevel.Nodes(0).Nodes(idx).Nodes(i).Nodes.Add(str)
                            '    node.Tag = dtTable.Rows(rw)("invtId").ToString()
                            '    'node.ImageIndex = dtTable.Rows(rw)("ImageIndex")
                            '    'node.SelectedImageIndex = dtTable.Rows(rw)("ImageIndex")
                            '    node.Name = str
                            '    node.Text = str
                            'End If
                        Next rw
    ...
    Private Sub TreeLevel_AfterSelect(sender As Object, e As TreeViewEventArgs) Handles TreeLevel.AfterSelect
        Try
            ...
            fc_Class.getDataByInvt(e.Node.Text)
            dt = fc_Class.getDetailBoM_Part(e.Node.Text)
