...
Public Class frmReport_BoM
    ...
    Sub LoadAllReport()
        Try
            ...
            ds = fc_class.reportAllBoM(cmbStatus.Text.Trim, cmbSite.Text.Trim)
            Dim laporan As New BoM_report
        ...
        End Try
    End Sub
    Sub LoadReportMultiLevelBoM()
        Try
            Dim ds As New DataSet
            ds = fc_class.Report_MultiLevelBoM(_txtInvtID2.Text)
            Dim laporan As New BoM_MultiLevel
            laporan.SetDataSource(ds.Tables("MultiLevelBoM"))
            Dim preview As New FrmSystem_PrintPreview1
            preview.CRViewer.ReportSource = laporan
            'preview.ViewReportC = crReport
            'preview.CRViewer.ShowLastPage()
    ...
    Sub ViewBoMRouting()
        Try
            Dim ds As New DataSet
            ds = fc_class.View_BoMRouting(cmbSite.Text.Trim, cmbStatus.Text.Trim, _txtInvID.Text.Trim)
            Dim laporan As New BoM_Routing
            laporan.SetDataSource(ds.Tables("BoM_Routing"))
            Dim preview As New FrmSystem_PrintPreview1
            preview.CRViewer.ReportSource = laporan
            'preview.ViewReportC = crReport
            'preview.CRViewer.ShowLastPage()
    ...
    Sub ViewMaterialUsedBy()
        Try
            Dim ds As New DataSet
            ds = fc_class.View_BoMRouting(cmbSite.Text.Trim, cmbStatus.Text.Trim, _txtInvID.Text.Trim)
            Dim laporan As New BoM_MaterialUsedBy
            laporan.SetDataSource(ds.Tables("BoM_Routing"))
            Dim preview As New FrmSystem_PrintPreview1
            preview.CRViewer.ReportSource = laporan
            'preview.ViewReportC = crReport
    ...
    Public Sub PrintDokumen(Optional ByVal fb_Preview As Boolean = False)
        Try
            ...
            ElseIf TabControl1.SelectedTab Is TabPage3 Then
                ...
                '    ViewMaterialUsedBy()
                'Else
                '    MsgBox("Grid Empty, Load data first !")
                '    Exit Sub
                'End If
                'Dim crReport As New CrystalDecisions.CrystalReports.Engine.ReportDocument 'Object
                'If Grid.Rows.Count > 0 Then
                '    crReport = fc_class.PrintBomRouting(cmbSite.Text, cmbStatus.Text, _txtInvID.Text, "BoM_Routing")
